...
function main() {
    ...
    if (assignment_id == 'ASSIGNMENT_ID_NOT_AVAILABLE') {
      ...
      if (typeof(condition) == "undefined") {
        ...
        if (typeof(condition_set) === "undefined") {
          ...
          // combined
          // stepthru
          // condition_set = 'static,combined,stepthru,stepthru_text_only';
          condition_set = 'static,stepthru';
          //condition_set = 'stepthru_text_only';
        }
    ...
    // ======== apply condition ========
    if (condition == "static") {
      $('#p_response').text("Your job is to explore how work settings and degrees relate to salaries in this survey. Therefore you created two tables, shown below, to inspect data from all 100 respondents. Each row in the table shows one person's degree, work setting, and salary.");
    } else if (condition == "combined") {
      $('#h2_combined').text("Inspecting survey data");
      $('#p_combined').text("You analyzed the responses from all 100 participants in two ways. First, you looked at how work settings alone relate to salaries for all 100 participants, shown in the movie on the left. Then, you looked at how work settings AND degree relate to salaries for all 100 participants, shown in the movie on the right. The rows shown in Step 3 of each movie show average salaries for each group.");

      $('#h2_response').text("Reflecting on your findings");
      $('#p_response').text("These tables are from the final step of your analysis, on how work settings and degrees relate to salaries. The rows shown here show average salaries for each group.")
    ...
    Promise
    .all([load_gif("img/full-degree-tibble.gif"),//degree
    load_gif("img/full-degree-industry-tibble.gif")])//degree and work
...
function log_start_of_experiment() {
	$.ajax({
		...
		//datatype: "json",
		data: JSON.stringify({
      study_id : "simpsons_multiple_choice_tibble",
			hitId: hit_id,
...
var answer_choices = {
  "non-grads-on-right" : "People with neither master's nor PhDs are factored into the right table, which biases the averages in opposing directions.",
  "mystery-work-on-left" : "The table on the left includes salaries from people who work in neither industry nor academia and who are unrepresentative of the general trend.",
  "many-industry-on-right" : "In the right table, there are many kinds of industry jobs, but fewer kinds of academic jobs.",
  "outliers-drive-up-masters" : "Due to outliers, the master's average salary in the left table can be higher than both master's average salaries in the right table.",
  "left-not-stat-sig" : "The differences in the left table are not statistically significant and therefore could be due to chance.",
  "left-more-than-right": "The left table shows data from more respondents than the right table, so it is not appropriate to compare the two tables.​​",
  ...
  "more-masters-in-industry" : "Most people with a master's degree work in industry, which pays more and drives up the average master's salary in the left table.",
